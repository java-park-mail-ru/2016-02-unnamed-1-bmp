...
public class GameMechanicsImpl implements GameMechanics {
    ...
    public boolean addUserForRandomGame(GameUser gameUser) {
        ...
        if (this.waiters.isEmpty()) {
            ...
            if (gameSession.addUser(gameUser)) {
                LOGGER.info("Add user to random game (to waiters), user {}, game session id {}", gameUser.getName(), gameSession.getId());
            ...
            if (gameSession.addUser(gameUser)) {
                LOGGER.info("Add user to random game, user {}, game session id {}", gameUser.getName(), gameSession.getId());
    ...
    public boolean addUserForBotGame(GameUser gameUser) {
        ...
        final GameSession gameSession = new GameSession(this.context, gameUser.getFieldProperties());
        if (gameSession.addUser(gameUser)) {
            LOGGER.info("Add user to bot game, user {}, game session id {}", gameUser.getName(), gameSession.getId());
            this.gameUsers.put(gameUser.getUser().getId(), gameUser);
            this.usersToSessions.put(gameUser.getUser().getId(), gameSession);
    ...
    public boolean addUserForFriendGame(GameUser gameUser, @Nullable Long gameSessionId) {
        LOGGER.info("Attempting to add user to friend game, user {}, game session id {}", gameUser.getName(), gameSessionId);
        if (gameUser.getUser() == null || this.hasUserGameSession(gameUser.getUser())) {
            this.webSocketService.notifyInitGame(gameUser, false, null);
            return false;
        }
        ...
        if (gameSessionId != null) {
            ...
            if (gameSession != null) {
                if (gameSession.addUser(gameUser)) {
                    LOGGER.info("Add user to friend game, user {}, game session id {}", gameUser.getName(), gameSession.getId());
                    final long userId = gameUser.getUser().getId();
                    this.gameUsers.put(userId, gameUser);
                    this.usersToSessions.put(userId, gameSession);
            ...
            final GameSession gameSession = new GameSession(this.context, gameUser.getFieldProperties());
            if (gameSession.addUser(gameUser)) {
                LOGGER.info("Add user to friend game (create new one), user {}, game session id {}", gameUser.getName(), gameSession.getId());
                this.gameUsers.put(gameUser.getUser().getId(), gameUser);
                this.gameSessions.put(gameSession.getId(), gameSession);
                this.usersToSessions.put(gameUser.getUser().getId(), gameSession);
